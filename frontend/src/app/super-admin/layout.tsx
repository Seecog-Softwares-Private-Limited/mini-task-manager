...
export default function SuperAdminLayout({ children }: { children: React.ReactNode }) {
  ...
  if (!ready || isLoading) {
    return <div className="p-6 text-sm text-muted-foreground">Loading super admin portal…</div>;
  }
  ...
  return (
    <div className="min-h-screen bg-background">
      <div className="mx-auto flex max-w-7xl gap-6 px-4 py-6">
        <aside className="sticky top-4 h-fit w-64 rounded-xl border bg-card p-3">
          ...
          <div className="mt-4 border-t pt-3">
            <Button variant="outline" size="sm" className="w-full justify-start gap-2" onClick={() => router.push("/dashboard")}>
              <LayoutDashboard className="h-4 w-4" />
              Back to App
            </Button>
            <Button variant="ghost" size="sm" className="mt-2 w-full justify-start gap-2" onClick={handleLogout}>
