...
let OrganizationsService = class OrganizationsService {
    ...
    async update(id, dto) {
        ...
        if (dto.name !== undefined) {
            ...
            if (!trimmed) {
                throw new common_1.BadRequestException('Workspace name cannot be empty');
            }
            patch.name = trimmed;
        }
        if (dto.slug !== undefined) {
            ...
            if (!nextSlug) {
                throw new common_1.BadRequestException('URL slug cannot be empty');
            }
            if (nextSlug !== org.slug) {
                const taken = await this.organizationsRepository.findBySlug(nextSlug);
                if (taken && taken.id !== id) {
                    throw new common_1.ConflictException('A workspace with this slug already exists. Please choose a different slug.');
                }
            }
            patch.slug = nextSlug;
        }
