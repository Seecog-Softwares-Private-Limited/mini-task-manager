...
1. **Config load** — `bootstrap-env` + `ConfigModule` load repo-root **`.env` only** (prefer not duplicating keys in `frontend/.env*`), then `configuration()` / `validate()`.  
...
| **Missing .env** | Wrong defaults (e.g. DB password), possible hardcoded fallbacks | Medium | app.js warns; remove hardcoded defaults in config (see P1). |
...
- **Remove hardcoded DB password in config** — `src/config/configuration.ts` has a default `DB_PASSWORD` fallback (`'Nikhil-700'`). Prefer no default or empty so missing `.env` is obvious; app.js already warns when DB_* are unset.
- **Ensure `.env` includes DB and JWT** — New clones create `.env` and set values.
- **Billing env documentation** — List RAZORPAY_KEY_ID, RAZORPAY_KEY_SECRET (and any webhook secret) in README so payment flows are configurable.
...
- **Next steps**: P1 (config defaults and `.env`) and P2 (CORS and billing env docs) give the highest benefit for minimal change.
